...
;; create card
(def card (e/native-card {:name "my new card 1"
                          :database (:id db)
...
(let [card1 (e/native-card {:name "my new card 1"
                            :database (:id db)
                            :sql "SELECT * FROM enzo.account"})
      card2 (e/native-card {:name "my new card 2"
                            :database (:id db)
                            :sql "SELECT * FROM enzo.currency"})
      dash (e/find-or-create! conn (e/dashboard {:name "Bar"
                                                 :cards [{:card card1
                                                          :x 0 :y 0
                                                          :width 5 :height 5}
                                                         {:card card2
